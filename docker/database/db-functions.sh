...
grant_permissions() {
  local db_name="$1"
  echo "[vmangos-deploy]: Granting permissions to database user $MARIADB_USER for $db_name"
  mariadb -u root -p"$MARIADB_ROOT_PASSWORD" -e \
...
import_dump() {
  local db_name="$1"
  local dump_file="$2"
  echo "[vmangos-deploy]: Importing initial data for $db_name"
...
import_updates() {
  ...
  if [ -e "$update_file" ]; then
    echo "[vmangos-deploy]: Importing potential updates for $db_name"
